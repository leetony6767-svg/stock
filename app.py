...
if admin_mode:
    ...
    st.markdown("<h1>請登入</h1>", unsafe_allow_html=True)  # 強制顯示標題
    username_input = st.text_input("帳號（Line ID 或手機號碼）")
    username = username_input.strip() if username_input else ""
    if st.button("登入"):
        st.markdown(f'<div class="debug-info">你輸入的帳號（去除空格後）："{username}"</div>', unsafe_allow_html=True)
        c.execute("SELECT expiry_date FROM users WHERE username=?", (username,))
        result = c.fetchone()
        st.markdown(f'<div class="debug-info">資料庫查詢結果：{result}</div>', unsafe_allow_html=True)
        if result:
            expiry_str = result[0]
            try:
                expiry = datetime.strptime(expiry_str, "%Y-%m-%d")
                st.markdown(f'<div class="debug-info">到期日：{expiry_str}</div>', unsafe_allow_html=True)
